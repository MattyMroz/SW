...
// --- Tablica wzorów segmentów dla cyfr 0-9 ---
const uint16_t digit_segments_map[] = { // Indeks 0-9 dla cyfr 0-9
...
#define NUM_SEGMENT_PATTERNS (sizeof(digit_segments_map) / sizeof(digit_segments_map[0]))

// --- Tablica masek pinów anod dla poszczególnych cyfr wyświetlacza ---
const uint16_t digit_anode_pins_mask[] = {
...
#define NUM_DISPLAY_DIGITS (sizeof(digit_anode_pins_mask) / sizeof(digit_anode_pins_mask[0]))


static uint16_t counter_value = 0;
static uint8_t current_display_digit_idx = 0; // Indeks aktualnie wyświetlanej cyfry (0 do NUM_DISPLAY_DIGITS-1)
// Tablica przechowująca cyfry do wyświetlenia (jedności, dziesiątki, etc.)
static uint8_t digits_to_display[NUM_DISPLAY_DIGITS] = {0, BLANK_DIGIT, BLANK_DIGIT, BLANK_DIGIT}; // Inicjalizacja dla 4 cyfr
...
void GPIO_Init_Display_Joystick(void) {
    ...
    // Zakładamy, że NUM_DISPLAY_DIGITS jest zgodne z liczbą zdefiniowanych DIGITx_PIN_NUM
...
// Aby ją uelastycznić za pomocą NUM_DISPLAY_DIGITS, wymagałaby większych zmian.
// Na razie pozostawiam ją w obecnej formie, zakładając NUM_DISPLAY_DIGITS = 4.
void Get_Digits_For_Display(uint16_t number, uint8_t* digits_array) {
    // Upewnij się, że digits_array ma co najmniej NUM_DISPLAY_DIGITS elementów
    if (NUM_DISPLAY_DIGITS < 4 && number > 0) { // Proste zabezpieczenie, jeśli ktoś zmieni NUM_DISPLAY_DIGITS na mniej niż 4
    ...
    if (NUM_DISPLAY_DIGITS > 1) digits_array[1] = (number / 10) % 10;
    if (NUM_DISPLAY_DIGITS > 2) digits_array[2] = (number / 100) % 10;
    if (NUM_DISPLAY_DIGITS > 3) digits_array[3] = (number / 1000) % 10;


    if (number == 0) {
        // Dla 0, wyświetl "   0" (lub odpowiednio dla NUM_DISPLAY_DIGITS)
        for(uint8_t i = 1; i < NUM_DISPLAY_DIGITS; ++i) {
    ...
    for(int i = NUM_DISPLAY_DIGITS - 1; i >= 1; i--)
    {
...
void Display_Digit(uint8_t digit_value, uint8_t digit_position) {
    ...
    if (digit_value < NUM_SEGMENT_PATTERNS) {
    ...
    if (digit_position < NUM_DISPLAY_DIGITS) {
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if ((current_tick - last_display_refresh_time) >= DISPLAY_REFRESH_DELAY_MS)
    {
        ...
        if (current_display_digit_idx >= NUM_DISPLAY_DIGITS) {
